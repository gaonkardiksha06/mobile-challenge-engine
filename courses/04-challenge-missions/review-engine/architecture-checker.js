  ...
  try {
    ...
    traverse(ast, {
      CallExpression(path) {
        ...
        if (calleeName === 'useNavigation') foundPatterns.add('useNavigation');
      ...
      JSXElement(path) {
        ...
        if (name === 'FlatList') foundPatterns.add('FlatList');
        if (name === 'ScrollView') foundPatterns.add('ScrollView');
        if (name === 'TextInput') foundPatterns.add('TextInput');
    ...
    if (patternsRequired.includes('useEffect')) {
      if (/useEffect\s*\(/.test(content)) fallbackFound.add('useEffect');
    }
    if (patternsRequired.includes('StyleSheet')) {
      if (/StyleSheet\.create/.test(content)) fallbackFound.add('StyleSheet');
    }
    if (patternsRequired.includes('FlatList')) {
      if (/<FlatList/.test(content)) fallbackFound.add('FlatList');
    }
    if (patternsRequired.includes('fetch')) {
      if (/\bfetch\s*\(/.test(content)) fallbackFound.add('fetch');
    }
    if (patternsRequired.includes('AsyncStorage')) {
      if (/AsyncStorage/.test(content)) fallbackFound.add('AsyncStorage');
    }
    if (patternsRequired.includes('TextInput')) {
      if (/<TextInput/.test(content)) fallbackFound.add('TextInput');
    }
    const mobilePatterns = ['redux', 'createSlice', 'firebase', 'express', 'mongoose', 'jwt', 'bcrypt', 'middleware', 'http', 'fs', 'nativewind', 'expo-notifications', 'react-native-maps', 'eas'];
    for (const mp of mobilePatterns) {
      if (patternsRequired.includes(mp) && content.toLowerCase().includes(mp.toLowerCase())) {
        fallbackFound.add(mp);
      }
    }
    for (const pattern of patternsRequired) {
